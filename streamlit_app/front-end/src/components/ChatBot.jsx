...
import robot_img from "../assets/robot_image.jpg";
...
function ChatBot(props) {
  ...
  let [sourceData, SetSourceData] = useState("fptu");
  ...
  const [dataChat, SetDataChat] = useState([
    [
      "start",
      [
        "Xin chào! Đây là FPTU Chatbot, trợ lý đắc lực dành cho bạn! Bạn muốn tìm kiếm thông tin về những gì? Hãy hỏi mình bất kỳ điều gì nhé! 😄",
        null,
      ],
    ],
  ]);
    ...
    if (promptInput !== "" && isLoading === false) {
      ...
      fetch("http://localhost:8000/query",
      {
        ...
        body: JSON.stringify({
          query: promptInput,
          top_k: 5,
          use_rerank: true  // GPU enabled - reranking nhanh!
        }),
      })
        .then((response) => {
        ...
        .then((result) => {
          ...
          const sources = result.citations ? result.citations.map(c => ({
            source: c.source,
            page: c.page,
            score: c.score
          })) : [];
          ...
          const answer = result.answer || "Xin lỗi, không có câu trả lời.";
          
          SetDataChat((prev) => [
            ...prev,
            ["start", [answer, sources, sourceData]],
          ]);
          SetIsLoad(false);
          SetIsGen(false);  // Re-enable input
        })
        .catch((error) => {
          console.error("API Error:", error);
          SetDataChat((prev) => [
            ...prev,
            ["start", [`Lỗi: ${error.message}. Vui lòng kiểm tra Backend API.`, []]],
          ]);
          SetIsLoad(false);
          SetIsGen(false);  // Re-enable input on error
        });
    }
  }
  ...
  const handleReferenceClick = (citation, sourceType) => {
    // citation = {source, page, score}
    SetReference({
      title: citation.source + (citation.page ? ` - Trang ${citation.page}` : ""),
      source: "Đại học FPT",
      url: "https://daihoc.fpt.edu.vn/",
      text: `Tài liệu: ${citation.source}\nTrang: ${citation.page || "N/A"}\nĐộ liên quan: ${(citation.score * 100).toFixed(1)}%`,
    });
  };
  return (
          ...
          {dataChat.map((dataMessages, i) =>
            dataMessages[0] === "start" ? (
              <div className="chat chat-start drop-shadow-md" key={i}>
                ...
                <div className="chat-bubble chat-bubble-info colo break-words prose prose-sm max-w-none">
                  <ReactMarkdown
                    ...
                    components={{
                      ...
                      // Style for tables
                      table: ({node, ...props}) => <table className="table-auto border-collapse my-2" {...props} />,
                      th: ({node, ...props}) => <th className="border px-2 py-1 font-bold" {...props} />,
                  ...
                  dataMessages[1][1].length == 0 ? (
                    ...
                    <>
                      ...
                      <p className="font-semibold text-xs">
                        Tham khảo:{" "}
                        {dataMessages[1][1].map((citation, j) => (
                          <label
                            htmlFor="my_modal_6"
                            className="kbd kbd-xs mr-1 hover:bg-sky-300 cursor-pointer"
                            onClick={() =>
                              handleReferenceClick(citation, dataMessages[1][2])
                            }
                            key={j}
                          >
                            {citation.source || "Tài liệu"}
